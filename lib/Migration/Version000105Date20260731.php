...
final class Version000105Date20260731 extends SimpleMigrationStep {
	public function changeSchema(IOutput $output, Closure $schemaClosure, array $options): ?ISchemaWrapper {
		$schema = $schemaClosure();
		if (!$schema->hasTable('proofing_presets')) {
			$table = $schema->createTable('proofing_presets');
			...
			$table->addUniqueIndex(['owner_uid', 'name'], 'proof_preset_owner_name');
		}
		return $schema;
	}
}

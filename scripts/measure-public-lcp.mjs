...
const coldBudgetMs = Number(process.env.LCP_COLD_BUDGET_MS ?? 12000)
const warmBudgetMs = Number(process.env.LCP_WARM_BUDGET_MS ?? 4000)
...
for (let round = 1; round <= rounds; round++) {
	...
	await client.send('Network.setCacheDisabled', { cacheDisabled: round === 1 })
	...
	results.push({
		round,
		phase: round === 1 ? 'cold-client' : 'warm-client',
...
const coldLcpMs = results[0]?.lcpMs ?? null
const warmValues = results.slice(1).map(result => result.lcpMs).filter(value => value !== null).sort((left, right) => left - right)
const warmMedianLcpMs = warmValues[Math.floor(warmValues.length / 2)] ?? coldLcpMs
const report = {
	target,
	...
	coldLcpMs,
	warmMedianLcpMs,
	budgets: { coldMs: coldBudgetMs, warmMs: warmBudgetMs },
	results,
}
await writeFile(path.join(outputDirectory, 'public-lcp-report.json'), `${JSON.stringify(report, null, 2)}\n`)
console.log(JSON.stringify(report, null, 2))
if (coldLcpMs === null || coldLcpMs > coldBudgetMs || warmMedianLcpMs === null || warmMedianLcpMs > warmBudgetMs) {
	throw new Error(`Public LCP budget failed: cold ${coldLcpMs ?? 'missing'}ms / ${coldBudgetMs}ms, warm ${warmMedianLcpMs ?? 'missing'}ms / ${warmBudgetMs}ms`)
}

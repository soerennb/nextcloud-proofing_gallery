...
const retryDelays = [250, 500, 1_000, 2_000, 4_000, 8_000, 12_000, 15_000]
const retryableMethods = new Set(['DELETE', 'GET', 'HEAD', 'MKCOL', 'OPTIONS', 'PROPFIND', 'PUT'])

/**
 * Nextcloud's Apache process can briefly close a connection while the
 * container finishes booting. Retry only idempotent setup requests so a lost
 * response cannot create duplicate galleries.
 */
async function e2eFetch(input: string | URL, init: RequestInit = {}): Promise<Response> {
	...
	for (let attempt = 0; attempt <= retryDelays.length; attempt++) {
		try {
			...
			if (!retryable || response.status < 500 || attempt === retryDelays.length) return response
			await response.body?.cancel()
			lastError = new Error(`HTTP ${response.status}`)
		} catch (error) {
			lastError = error
			if (!retryable || attempt === retryDelays.length) {
				throw new Error(`E2E setup request ${method} ${url} failed after ${attempt + 1} attempts`, { cause: error })
			}
		}
		await new Promise(resolve => setTimeout(resolve, retryDelays[attempt]))
	}

	throw new Error(`E2E setup request ${method} ${url} failed`, { cause: lastError })
}

export default async function globalSetup(config: FullConfig) {
	const baseURL = String(config.projects[0].use.baseURL)
	const headers = { Authorization: auth, 'OCS-APIRequest': 'true' }
	// Wait for the HTTP stack itself, not just the Nextcloud CLI bootstrap used
	...
	const preferences = await e2eFetch(`${baseURL}/ocs/v2.php/apps/proofing_gallery/api/v1/user/preferences?format=json`, {
		method: 'PUT',
	...
	for (const [, href] of fixtureContents.matchAll(/<(?:d:)?href>([^<]+)<\/(?:d:)?href>/g)) {
		...
		if (pathname.replace(/\/?$/, '/') !== fixtureRoot) {
			await e2eFetch(new URL(pathname, baseURL), { method: 'DELETE', headers })
		}
	}
	...
	await e2eFetch(`${dav}/proof.png`, {
		method: 'PUT',
		headers: { ...headers, 'Content-Type': 'image/png' },
		body: png,
	})
	const propfind = await e2eFetch(dav, {
		method: 'PROPFIND',
	...
	const galleries = await e2eFetch(galleriesUrl, { headers }).then(response => response.json()) as {
	...
	for (const stale of galleries.items.filter(item => item.title.startsWith('E2E '))) {
		await e2eFetch(`${baseURL}/ocs/v2.php/apps/proofing_gallery/api/v1/galleries/${stale.id}?format=json`, {
			method: 'DELETE',
			headers,
		})
	}
	const response = await e2eFetch(galleriesUrl, {
		method: 'POST',
		headers: { ...headers, 'Content-Type': 'application/json' },
		body: JSON.stringify({
			folderId,
	...
	await e2eFetch(largeDav, { method: 'MKCOL', headers })
	const largeContents = await e2eFetch(largeDav, {
		method: 'PROPFIND',
		headers: { ...headers, Depth: '1', 'Content-Type': 'application/xml' },
		body: '<?xml version="1.0"?><d:propfind xmlns:d="DAV:"><d:prop><d:resourcetype/></d:prop></d:propfind>',
	...
	await Promise.all([...largeContents.matchAll(/<(?:d:)?href>([^<]+)<\/(?:d:)?href>/g)].map(async ([, href]) => {
		const pathname = new URL(href, baseURL).pathname
		if (pathname.replace(/\/?$/, '/') !== largeRoot) await e2eFetch(new URL(pathname, baseURL), { method: 'DELETE', headers })
	}))
	...
	for (let offset = 0; offset < largeImages.length; offset += 25) {
		await Promise.all(largeImages.slice(offset, offset + 25).map((body, index) => e2eFetch(`${largeDav}/mobile-${String(offset + index + 1).padStart(largeNameWidth, '0')}.${largeExtension}`, {
			method: 'PUT',
			headers: { ...headers, 'Content-Type': useLocalFixtures ? 'image/webp' : 'image/png' },
			body,
		})))
	}
	const largePropfind = await e2eFetch(largeDav, {
		method: 'PROPFIND',
		headers: { ...headers, Depth: '0', 'Content-Type': 'application/xml' },
		body: '<?xml version="1.0"?><d:propfind xmlns:d="DAV:" xmlns:oc="http://owncloud.org/ns"><d:prop><oc:fileid/></d:prop></d:propfind>',
	})
	...
	for (let index = 1; index <= 4; index++) {
		const dashboardGallery = await e2eFetch(galleriesUrl, {
			method: 'POST',
			headers: { ...headers, 'Content-Type': 'application/json' },
			body: JSON.stringify({
				folderId,

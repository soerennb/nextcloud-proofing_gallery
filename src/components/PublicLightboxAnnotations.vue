...
const markers = computed(() => [...props.comments]
	.sort((left, right) => left.createdAt - right.createdAt || left.id - right.id)
	.flatMap(comment => comment.annotations.map((annotation, annotationIndex) => ({ comment, annotation, annotationIndex })))
...
<template>
	<Teleport v-if="host" :to="host">
		<button v-for="marker in markers"
			:key="`${marker.comment.id}-${marker.annotationIndex}`"
			type="button"
			class="annotation-marker"
			:class="{ 'annotation-marker--selected': selectedCommentId === marker.comment.id }"
			...
			:aria-controls="`point-comment-${marker.comment.id}`"
			@pointerdown.stop
...
<style scoped>
.annotation-marker { position: absolute; z-index: 2; display: grid; width: 28px; height: 28px; padding: 0; border: 2px solid #fff; border-radius: 50%; background: var(--gallery-accent); box-shadow: 0 0 0 3px rgb(0 0 0 / 66%); color: #fff; font-size: 12px; font-weight: 750; line-height: 1; place-items: center; pointer-events: auto; transform: translate(-50%, -50%) scale(var(--annotation-marker-scale, 1)); transform-origin: center; }
...
.annotation-composer button { min-height: 38px; padding: 0 14px; border: 0; border-radius: 8px; background: var(--gallery-accent); color: #fff; font: inherit; font-weight: 650; }
...
@media (max-width: 760px) { .annotation-composer { inset: auto 12px max(12px, env(safe-area-inset-bottom)); width: auto; transform: none; } }
</style>

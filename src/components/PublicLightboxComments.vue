...
<template>
	<ul class="comment-list" :class="{ 'comment-list--focused': selectedCommentId !== null }">
		<li v-for="comment in comments"
			...
			<header class="comment-list__header">
				...
				<div>
					<span class="comment-list__identity">
						...
						<span v-if="comment.mine && comment.deletedAt === null && editingCommentId !== comment.id" class="comment-actions">
							<button type="button"
			...
			<p v-else>
				{{ comment.deletedAt === null ? comment.body : t('proofing_gallery', 'Comment deleted') }}
			</p>
		</li>
	</ul>
</template>

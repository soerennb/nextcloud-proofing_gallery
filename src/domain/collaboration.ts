...
export function mergeCollaborationState(current: CollaborationState, incoming: CollaborationState, hydratedIds: number[]): CollaborationState {
	...
	const deletedSelectionIds = new Set((incoming.events ?? [])
		.filter(event => event.type === 'selection.deleted')
		.map(event => event.payload.selectionId)
	...
	return {
		...
		comments,
		selections: [...current.selections.filter(selection => !deletedSelectionIds.has(selection.id) && !incoming.selections.some(value => value.id === selection.id)), ...incoming.selections],
